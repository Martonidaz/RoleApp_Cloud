...
exports.getConfig = async (req, res) => {
    try {
        ...
        // Rota segura para enviar a chave APENAS para o mapa do frontend
        // O frontend deve usar isso para carregar o script do Maps
        config.googleMapsKey = GOOGLE_API_KEY; 
...
// NOVO: Função para esconder a chave ao carregar imagens
exports.getFotoProxy = async (req, res) => {
    const { ref } = req.query;
    if (!ref || !GOOGLE_API_KEY) return res.status(404).send("Sem imagem ou chave");

    try {
        const googleUrl = `https://places.googleapis.com/v1/${ref}/media?key=${GOOGLE_API_KEY}&maxHeightPx=400&maxWidthPx=400`;
        
        // Busca a imagem no Google (Server-to-Server)
        const response = await axios.get(googleUrl, { 
            responseType: 'stream',
            // Adicionamos o Referer para caso a chave tenha restrição de domínio
            headers: { 'Referer': 'http://localhost:3000' } 
        });
        
        // Repassa a imagem para o frontend
    ...
    try {
        ...
        const response = await axios.post(
            ...
            { 
                headers: { 
                    ...
                    // TRUQUE: Envia o Referer para que a chave funcione mesmo se estiver restrita a localhost no console
        ...
        let locais = (response.data.places || []).map(l => {
            ...
            if (l.photos && l.photos.length > 0) {
                // SEGURANÇA: Aqui mudamos para usar a rota Proxy.
                // A URL final será algo como: http://localhost:3000/api/foto?ref=places/photos/....
                // A chave NÃO vai para o frontend.
                fotoUrl = `${req.protocol}://${req.get('host')}/api/foto?ref=${l.photos[0].name}`;
            }
            return {
                id: l.id,
